...
#Open the data frame with both the genetic/biochemical and the resistance
print("Opening data files")
with open('pcaGenomicBiochemicalDataFrameTrainingTesting.csv', newline='') as f:
    reader = csv.reader(f)
    genomicBiochemicalPCADataFrame = list(reader)
...
for i in range(len(genomicBiochemicalPCADataFrame)):
     genomicBiochemicalPCADataFrame[i] = list(map(float, genomicBiochemicalPCADataFrame[i]))
...
genomicBiochemicalPCATrainingDat = genomicBiochemicalPCADataFrame[1:int(len(genomicBiochemicalPCADataFrame)/2)]
genomicBiochemicalPCATestingDat = genomicBiochemicalPCADataFrame[int(len(genomicBiochemicalPCADataFrame)/2)+1 :
                                    len(genomicBiochemicalPCADataFrame)]
     ...
     with open('PredictionsOfEachKernel/'+ k+'_pca.csv', 'w', newline='') as csvfile:
          ...
          for c in (1,10,100):
               for deg in (3,5,7,9):
                    ...
                    clf.fit(genomicBiochemicalPCATrainingDat,resisTrainingData)

                    trainDatPredictions = list(clf.predict(genomicBiochemicalPCATrainingDat))
                    testDatPredictions = list(clf.predict(genomicBiochemicalPCATestingDat))
...
clf2 = clf2.fit(genomicBiochemicalPCATrainingDat,resisTrainingData)

trainDatPredictions = list(clf2.predict(genomicBiochemicalPCATrainingDat))
testDatPredictions = list(clf2.predict(genomicBiochemicalPCATestingDat))
...
X = genomicBiochemicalPCADataFrame
...
models = (svm.SVC(kernel='sigmoid', C=10),
          svm.SVC(kernel='poly', C=10, degree = 7))
