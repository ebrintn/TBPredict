...
with open('pcaBiochemicalDataFrameTrainingTesting.csv', newline='') as f:
    reader = csv.reader(f)
    biochemistryPCADataFrame = list(reader)
...
for i in range(len(biochemistryPCADataFrame)):
     biochemistryPCADataFrame[i] = list(map(float, biochemistryPCADataFrame[i]))
...
biochemPCATrainingData = biochemistryPCADataFrame[1:int(len(biochemistryPCADataFrame)/2)]
biochemPCATestingData = biochemistryPCADataFrame[int(len(biochemistryPCADataFrame)/2)+1 :
                                    len(biochemistryPCADataFrame)]
     ...
     # with open('PredictionsOfEachKernel/'+ k+'_pca.csv', 'w', newline='') as csvfile:
          ...
          # for c in (1,10,100):
               # for deg in (3,5,7,9):
                    ...
                    # clf.fit(biochemPCATrainingData,resisTrainingData)

                    # trainDatPredictions = list(clf.predict(biochemPCATrainingData))
                    # testDatPredictions = list(clf.predict(biochemPCATestingData))
...
# clf2 = tree.DecisionTreeClassifier(min_impurity_decrease = 0.1)
# clf2 = clf2.fit(biochemPCATrainingData,resisTrainingData)

# trainDatPredictions = list(clf2.predict(biochemPCATrainingData))
# testDatPredictions = list(clf2.predict(biochemPCATestingData))
